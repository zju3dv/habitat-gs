#!/usr/bin/env bash
#
# One-click DDPPO ObjectNav training on Gaussian Splatting scenes.
#
# Usage:
#   bash scripts_gs/train_objectnav.sh --output /path/to/output
#   bash scripts_gs/train_objectnav.sh --output /path/to/output --num-envs 8 --num-gpus 2
#
set -euo pipefail
...
if [[ -z "$OUTPUT_DIR" ]]; then
    echo "Usage: bash scripts_gs/train_objectnav.sh --output /path/to/output [options]"
    echo ""
    echo "Options:"
...
if [[ ! -d "data/scene_datasets/gs_scenes/episodes/objectnav/train/content" ]]; then
    echo "ERROR: ObjectNav training episode data not found."
...
echo "=========================================="
echo " DDPPO ObjectNav Training (GS Scenes)"
echo "=========================================="
...
if [[ $NUM_GPUS -gt 1 ]]; then
    python -m torch.distributed.launch \
        ...
        scripts_gs/run_objectnav.py \
        "${OVERRIDES[@]}" \
        "${EXTRA_ARGS[@]}"
else
    python -u scripts_gs/run_objectnav.py \
        "${OVERRIDES[@]}" \
        "${EXTRA_ARGS[@]}"
fi
