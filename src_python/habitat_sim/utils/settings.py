...
default_sim_settings: Dict[str, Any] = {
    ...
    # random seed
    "seed": 1,
    # initial gaussian time cursor (seconds) for Gaussian splatting scenes
    "gaussian_time": 0.0,
    # auto-play Gaussian time for dynamic scenes
    "gaussian_auto_play": True,
    # optional playback bounds/step
    "gaussian_time_min": 0.0,
    "gaussian_time_max": 0.0,
    "gaussian_time_step": 0.0,
    "gaussian_time_loop": True,
...
def make_cfg(settings: Dict[str, Any]):
    ...
    if "gaussian_time" in settings:
        sim_cfg.gaussian_time = settings["gaussian_time"]
    if "gaussian_auto_play" in settings:
        sim_cfg.gaussian_auto_play = settings["gaussian_auto_play"]
    if "gaussian_time_min" in settings:
        sim_cfg.gaussian_time_min = settings["gaussian_time_min"]
    if "gaussian_time_max" in settings:
        sim_cfg.gaussian_time_max = settings["gaussian_time_max"]
    if "gaussian_time_step" in settings:
        sim_cfg.gaussian_time_step = settings["gaussian_time_step"]
    if "gaussian_time_loop" in settings:
        sim_cfg.gaussian_time_loop = settings["gaussian_time_loop"]
