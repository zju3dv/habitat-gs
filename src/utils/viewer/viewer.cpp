...
-------------
  ...
  Mn::Text::GlyphCacheGL fontGlyphCache_{Mn::PixelFormat::R8Unorm,
                                         Mn::Vector2i{256}};
  std::unique_ptr<Mn::Text::Renderer2D> fontText_;
  Mn::Shaders::VectorGL2D fontShader_;
  bool showFPS_ = true;
  std::set<Key> pressedKeys_;
...
Viewer::Viewer(const Arguments& arguments)
     ...
     for best crispness. */
  {
    ...
    font_->fillGlyphCache(fontGlyphCache_,
                          ...
                          "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                          "0123456789:-_+,.! %µ");
    fontText_ = std::make_unique<Mn::Text::Renderer2D>(
        *font_, fontGlyphCache_, font_->size(), Mn::Text::Alignment::TopLeft);
    fontText_->reserve(512, Mn::GL::BufferUsage::DynamicDraw,
...
void Viewer::drawEvent() {
  ...
  if (showFPS_) {
    ...
    const auto glyphCapacity = static_cast<Mn::UnsignedInt>(text.size() + 32);
    if (fontText_->capacity() < glyphCapacity) {
      fontText_->reserve(glyphCapacity, Mn::GL::BufferUsage::DynamicDraw,
                         Mn::GL::BufferUsage::StaticDraw);
    }
