...
using attributes::GaussianAvatarAttributes;
...
void SceneInstanceAttributesManager::setValsFromJSONDoc(
  ...
  // Check for Gaussian Avatar instances existence
  const char* avatarTag = "gaussian_avatars";
  io::JsonGenericValue::ConstMemberIterator avatarJSONIter =
      jsonConfig.FindMember(avatarTag);
  if (avatarJSONIter == jsonConfig.MemberEnd()) {
    avatarTag = "avatars";
    avatarJSONIter = jsonConfig.FindMember(avatarTag);
  }
  if (avatarJSONIter != jsonConfig.MemberEnd()) {
    if (avatarJSONIter->value.IsArray()) {
      const auto& avatarArray = avatarJSONIter->value;
      for (rapidjson::SizeType i = 0; i < avatarArray.Size(); ++i) {
        const auto& avatarCell = avatarArray[i];
        if (avatarCell.IsObject()) {
          attribs->addGaussianAvatarInstanceAttrs(
              createGaussianAvatarAttributesFromJSON(avatarCell),
              validateUnique);
        } else {
          ESP_WARNING(Mn::Debug::Flag::NoSpace)
              << "Gaussian Avatar specification error in Scene Instance `"
              << attribsDispName << "` at idx : " << i
              << ": JSON cell within `" << avatarTag
              << "` array is not a valid "
                 "JSON object, so skipping entry.";
        }
      }
    } else {
      ESP_WARNING(Mn::Debug::Flag::NoSpace)
          << "Gaussian Avatar instances issue in Scene Instance `"
          << attribsDispName
          << "`: JSON cell `" << avatarTag
          << "` is not a valid JSON array, so no "
             "avatars loaded.";
    }
  } else {
    ESP_VERY_VERBOSE(Mn::Debug::Flag::NoSpace)
        << "No Gaussian Avatars specified for Scene Instance `"
        << attribsDispName
        << "`: JSON cell with tag `" << avatarTag
  ...
  io::jsonIntoSetter<double>(
      jsonConfig, "time_max",
      [attribs](double value) { attribs->setTimeMax(value); });
  io::jsonIntoSetter<bool>(
      jsonConfig, "time_loop",
      [attribs](bool value) { attribs->setTimeLoop(value); });
...
GaussianAvatarAttributes::ptr
SceneInstanceAttributesManager::createGaussianAvatarAttributesFromJSON(
    const io::JsonGenericValue& jCell) {
  GaussianAvatarAttributes::ptr avatarAttrs =
      GaussianAvatarAttributes::create("");

  std::string tmpVal;
  if (io::readMember<std::string>(jCell, "name", tmpVal)) {
    avatarAttrs->setHandle(tmpVal);
  }

  if (io::readMember<std::string>(jCell, "canonical_gaussians", tmpVal)) {
    avatarAttrs->setCanonicalGaussians(tmpVal);
  }

  // driver block
  auto driverIter = jCell.FindMember("driver");
  if (driverIter != jCell.MemberEnd()) {
    if (driverIter->value.IsString()) {
      avatarAttrs->setDriverSmplParams(driverIter->value.GetString());
    } else if (driverIter->value.IsObject()) {
      const auto& driver = driverIter->value;
      if (io::readMember<std::string>(driver, "smpl_params", tmpVal) ||
          io::readMember<std::string>(driver, "smpl_parms", tmpVal)) {
        avatarAttrs->setDriverSmplParams(tmpVal);
      }
      if (io::readMember<std::string>(driver, "trajectory", tmpVal)) {
        avatarAttrs->setDriverTrajectory(tmpVal);
      }
    }
  }

  if (io::readMember<std::string>(jCell, "smpl_model_path", tmpVal)) {
    avatarAttrs->setSmplModelPath(tmpVal);
  }
  if (jCell.HasMember("proxy_urdf")) {
    ESP_WARNING(Mn::Debug::Flag::NoSpace)
        << "Gaussian Avatar config contains deprecated field `proxy_urdf`; "
           "it is ignored. Use precomputed `proxy_capsules` in driver .pkl "
           "instead.";
  }
  if (io::readMember<std::string>(jCell, "smpl_type", tmpVal)) {
    avatarAttrs->setSmplType(tmpVal);
  }
  io::jsonIntoSetter<double>(
      jCell, "scale",
      [avatarAttrs](double scale) { avatarAttrs->setScale(scale); });
  io::jsonIntoSetter<double>(
      jCell, "offset_y",
      [avatarAttrs](double offset) { avatarAttrs->setOffsetY(offset); });
  io::jsonIntoSetter<double>(
      jCell, "time_begin",
      [avatarAttrs](double value) { avatarAttrs->setTimeBegin(value); });
  io::jsonIntoSetter<double>(
      jCell, "time_end",
      [avatarAttrs](double value) { avatarAttrs->setTimeEnd(value); });
