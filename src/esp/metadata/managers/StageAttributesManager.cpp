  ...
  if (!builtFromConfig) {
    ...
    if (attributesHandle != "NONE") {
      ...
      if (newAttributes->getNavmeshAssetHandle().empty()) {
        ...
        // if present, file was found, so set value (overriding attributes default)
...
void StageAttributesManager::setDefaultAssetNameBasedAttributes(
  ...
  up = up1;
  fwd = fwd1;
  const auto fileNameView = Cr::Containers::StringView{fileName};
  if (fileNameView.hasSuffix("_semantic.ply")) {
    assetTypeSetter(AssetType::InstanceMesh);
  } else if (fileNameView.hasSuffix(".gs.ply") ||
             fileNameView.hasSuffix(".3dgs.ply") ||
             fileNameView.hasSuffix(".4dgs.ply") ||
             fileNameView.hasSuffix(".4dgs.npz")) {
    assetTypeSetter(AssetType::GaussianSplatting);
  } else if (fileNameView.hasSuffix(".glb")) {
...
void StageAttributesManager::setValsFromJSONDoc(
  ...
  io::jsonIntoSetter<bool>(jsonConfig, "norm_quaternion",
                           [stageAttributes](bool value) {
                             stageAttributes->setGaussianNormalizeQuaternion(
                                 value);
                           });
