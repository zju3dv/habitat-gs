...
/**
 * @brief Attributes describing a Gaussian Avatar instance.
 */
class GaussianAvatarAttributes : public AbstractAttributes {
 public:
  explicit GaussianAvatarAttributes(const std::string& handle)
      : AbstractAttributes("GaussianAvatarAttributes", handle) {
    init("canonical_gaussians", "");
    init("driver_smpl_params", "");
    init("driver_trajectory", "");
    init("smpl_model_path", "");
    init("smpl_type", "");
    init("scale", 1.0);
    init("offset_y", 0.0);
    init("time_begin", 0.0);
  ...
  void setCanonicalGaussians(const std::string& path) {
    set("canonical_gaussians", path);
  }
  std::string getCanonicalGaussians() const {
    return get<std::string>("canonical_gaussians");
  }

  void setDriverSmplParams(const std::string& path) {
    set("driver_smpl_params", path);
  }
  std::string getDriverSmplParams() const {
    return get<std::string>("driver_smpl_params");
  }

  void setDriverTrajectory(const std::string& path) {
    set("driver_trajectory", path);
  }
  std::string getDriverTrajectory() const {
    return get<std::string>("driver_trajectory");
  }

  void setSmplModelPath(const std::string& path) {
    set("smpl_model_path", path);
  }
  std::string getSmplModelPath() const {
    return get<std::string>("smpl_model_path");
  }

  void setSmplType(const std::string& type) { set("smpl_type", type); }
  std::string getSmplType() const { return get<std::string>("smpl_type"); }

  void setScale(double scale) { set("scale", scale); }
  double getScale() const { return get<double>("scale"); }

  void setOffsetY(double offset) { set("offset_y", offset); }
  double getOffsetY() const { return get<double>("offset_y"); }

  void setTimeBegin(double time) { set("time_begin", time); }
  double getTimeBegin() const { return get<double>("time_begin"); }

  void setTimeEnd(double time) { set("time_end", time); }
  double getTimeEnd() const { return get<double>("time_end"); }

  ESP_SMART_POINTERS(GaussianAvatarAttributes)
};  // class GaussianAvatarAttributes
...
class SceneInstanceAttributes : public AbstractAttributes {
 public:
  ...
  /**
   * @brief Set the total scene time for Gaussian playback (seconds).
   */
  void setTimeMax(double time) { set("time_max", time); }
  /**
   * @brief Get the total scene time for Gaussian playback (seconds).
   */
  double getTimeMax() const { return get<double>("time_max"); }

  /**
   * @brief Set whether scene time loops when reaching time_max.
   */
  void setTimeLoop(bool loop) { set("time_loop", loop); }
  /**
   * @brief Get whether scene time loops when reaching time_max.
   */
  bool getTimeLoop() const { return get<bool>("time_loop"); }
  ...
  /**
   * @brief Add a Gaussian Avatar instance to this scene instance.
   */
  bool addGaussianAvatarInstanceAttrs(
      GaussianAvatarAttributes::ptr _avatarInstance,
      bool _validateUnique) {
    return setSubAttributesInternal<GaussianAvatarAttributes>(
        _avatarInstance, availableGaussianAvatarIDs_, gaussianAvatarConfig_,
        "gaussian_avatar_", _validateUnique);
  }

  /**
   * @brief Get the Gaussian Avatar instances for this scene.
   */
  std::vector<GaussianAvatarAttributes::cptr> getGaussianAvatarInstances()
      const {
    return getSubAttributesListInternal<GaussianAvatarAttributes>(
        gaussianAvatarConfig_);
  }

  /**
   * @brief Return the number of Gaussian Avatar instances.
   */
  int getNumGaussianAvatarInstances() const {
    return getNumSubAttributesInternal("gaussian_avatar_", gaussianAvatarConfig_);
  }

  /**
   * @brief Clear all Gaussian Avatar instances.
   */
  void clearGaussianAvatarInstances() {
    this->removeSubconfig("gaussian_avatars");
    gaussianAvatarConfig_ = editSubconfig<Configuration>("gaussian_avatars");
  }
 ...
 protected:
  ...
  /**
   * @brief Smartpointer to created Gaussian Avatar instance configuration.
   */
  std::shared_ptr<Configuration> gaussianAvatarConfig_{};

  /**
   * @brief Deque holding all released IDs to consume for Gaussian Avatar
   * instances when one is deleted.
   */
  std::deque<int> availableGaussianAvatarIDs_;
