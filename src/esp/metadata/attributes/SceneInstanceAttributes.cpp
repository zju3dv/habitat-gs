...
SceneInstanceAttributes::SceneInstanceAttributes(
    ...
    : AbstractAttributes(otr),
      ...
      availableGaussianAvatarIDs_(otr.availableGaussianAvatarIDs_) {
  // get refs to internal subconfigs for object and ao instances
  objInstConfig_ = editSubconfig<Configuration>("object_instances");
  ...
  copySubconfigIntoMe<SceneAOInstanceAttributes>(otr.artObjInstConfig_,
                                                 artObjInstConfig_);
  gaussianAvatarConfig_ = editSubconfig<Configuration>("gaussian_avatars");
  copySubconfigIntoMe<GaussianAvatarAttributes>(otr.gaussianAvatarConfig_,
...
SceneInstanceAttributes::SceneInstanceAttributes(
    ...
    : AbstractAttributes(std::move(static_cast<AbstractAttributes&&>(otr))),
      ...
      availableGaussianAvatarIDs_(std::move(otr.availableGaussianAvatarIDs_)) {
  // get refs to internal subconfigs for object and ao instances
...
void SceneInstanceAttributes::writeValuesToJson(
  ...
  writeValueToJson("time_max", jsonObj, allocator);
  writeValueToJson("time_loop", jsonObj, allocator);
...
void SceneInstanceAttributes::writeSubconfigsToJson(
  ...
  // gaussian avatar instances
  auto avatarCfgIterPair = gaussianAvatarConfig_->getSubconfigIterator();
  io::JsonGenericValue avatarArray(rapidjson::kArrayType);
  for (auto& cfgIter = avatarCfgIterPair.first;
       cfgIter != avatarCfgIterPair.second; ++cfgIter) {
    avatarArray.PushBack(cfgIter->second->writeToJsonObject(allocator),
                         allocator);
  }
  jsonObj.AddMember("gaussian_avatars", avatarArray, allocator);
  ...
  for (auto& cfgIter = cfgIterPair.first; cfgIter != cfgIterPair.second;
    ...
    if ((cfgIter->first == "object_instances") ||
        ...
        (cfgIter->first == "gaussian_avatars")) {
      continue;
    }
