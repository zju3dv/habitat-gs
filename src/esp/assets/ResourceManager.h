...
class ResourceManager {
 public:
   ...
   * @return whether built successfully or not
   */
  // Removed: Gaussian collision from filtered splats. Collision for Gaussian
  // stages should be provided explicitly via stage 'collision_asset'.
 ...
 public:
  ...
  /**
   * @brief Get metadata parsed from a Gaussian asset, if available.
   */
  Corrade::Containers::Optional<GaussianMetadata> getGaussianMetadata(
 ...
 protected:
  ...
  /**
   * @brief 3D Gaussian Splatting backend for createRenderAssetInstance
   */
  ...
  /**
   * @brief Cached Gaussian metadata keyed by render asset path.
   */
  std::unordered_map<std::string, GaussianMetadata> gaussianMetadataCache_;
