...
AssetInfo AssetInfo::fromPath(const std::string& path) {
  ...
  const auto pathView = Cr::Containers::StringView{path};

  if (pathView.hasSuffix("_semantic.ply")) {
    info.type = metadata::attributes::AssetType::InstanceMesh;
  } else if (pathView.hasSuffix(".gs.ply") ||
             pathView.hasSuffix(".3dgs.ply") ||
             pathView.hasSuffix(".4dgs.ply") ||
             pathView.hasSuffix(".4dgs.npz")) {
    // The name of gs asset is COMPULSORY! MUST ends with ".gs.ply",
    // ".3dgs.ply", ".4dgs.ply", or ".4dgs.npz"
    info.type = metadata::attributes::AssetType::GaussianSplatting;
  } else if (pathView.hasSuffix(".glb")) {
