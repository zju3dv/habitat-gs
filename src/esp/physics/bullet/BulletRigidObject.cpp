...
void BulletRigidObject::setBoxCollisionHalfExtents(
    const Magnum::Vector3& halfExtents) {
  btBoxShape* boxShape = nullptr;
  if (auto* directBox = dynamic_cast<btBoxShape*>(bObjectShape_.get())) {
    boxShape = directBox;
  } else if (bObjectShape_ && bObjectShape_->getNumChildShapes() > 0) {
    boxShape =
        dynamic_cast<btBoxShape*>(bObjectShape_->getChildShape(0));
  }
  if (!boxShape) {
    return;
  }
  const btVector3 halfBt(halfExtents.x(), halfExtents.y(), halfExtents.z());
  boxShape->setImplicitShapeDimensions(halfBt);
  if (isCollidable_) {
    bObjectRigidBody_->setCollisionShape(bObjectShape_.get());
