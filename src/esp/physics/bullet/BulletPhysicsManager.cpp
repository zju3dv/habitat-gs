...
inline Magnum::Vector3 toMagnum(const btVector3& v) {
  return Magnum::Vector3(v.x(), v.y(), v.z());
}

void appendCapsulesFromShape(const btCollisionShape* shape,
                             const btTransform& world,
                             std::vector<CapsuleObstacle>& out) {
  if (!shape) {
    return;
  }
  ...
  if (shape->isCompound()) {
    ...
    for (int i = 0; i < childCount; ++i) {
      ...
      appendCapsulesFromShape(compound->getChildShape(i),
                              world * childTransform, out);
    }
    return;
  }

  const auto* capsule = dynamic_cast<const btCapsuleShape*>(shape);
  if (capsule) {
    const int axis = capsule->getUpAxis();
    btScalar halfHeight = capsule->getHalfHeight();
    btScalar radius = capsule->getRadius();

    const btVector3 scale = capsule->getLocalScaling();
    btScalar axisScale = 1.0;
    btScalar radiusScale = 1.0;
    if (axis == 0) {
      axisScale = scale.x();
      radiusScale = btMax(scale.y(), scale.z());
    } else if (axis == 1) {
      axisScale = scale.y();
      radiusScale = btMax(scale.x(), scale.z());
    } else {
      axisScale = scale.z();
      radiusScale = btMax(scale.x(), scale.y());
    }
    halfHeight *= axisScale;
    radius *= radiusScale;

    btVector3 localA(0, 0, 0);
    btVector3 localB(0, 0, 0);
    if (axis == 0) {
      localA = btVector3(-halfHeight, 0, 0);
      localB = btVector3(halfHeight, 0, 0);
    } else if (axis == 1) {
      localA = btVector3(0, -halfHeight, 0);
      localB = btVector3(0, halfHeight, 0);
    } else {
      localA = btVector3(0, 0, -halfHeight);
      localB = btVector3(0, 0, halfHeight);
    }

    CapsuleObstacle obs;
    obs.p0 = toMagnum(world * localA);
    obs.p1 = toMagnum(world * localB);
    obs.radius = static_cast<float>(radius);
    out.push_back(obs);
    return;
  }

  const auto* cylinder = dynamic_cast<const btCylinderShape*>(shape);
  if (cylinder) {
    const btVector3 halfExtents = cylinder->getHalfExtentsWithMargin();
    int axis = cylinder->getUpAxis();
    btScalar halfHeight = 0.0;
    btScalar radius = 0.0;
    if (axis == 0) {
      halfHeight = halfExtents.x();
      radius = btMax(halfExtents.y(), halfExtents.z());
    } else if (axis == 1) {
      halfHeight = halfExtents.y();
      radius = btMax(halfExtents.x(), halfExtents.z());
    } else {
      halfHeight = halfExtents.z();
      radius = btMax(halfExtents.x(), halfExtents.y());
    }
...
std::vector<CapsuleObstacle>
BulletPhysicsManager::getKinematicCapsuleObstacles() const {
  std::vector<CapsuleObstacle> obstacles;
  if (!bWorld_) {
    return obstacles;
  }

  const int numObjects = bWorld_->getNumCollisionObjects();
  obstacles.reserve(static_cast<size_t>(numObjects));

  for (int i = 0; i < numObjects; ++i) {
    btCollisionObject* obj = bWorld_->getCollisionObjectArray()[i];
    if (!obj) {
      continue;
    }

    int objectId = ID_UNDEFINED;
    if (collisionObjToObjIds_) {
      auto mapIter = collisionObjToObjIds_->find(obj);
      if (mapIter != collisionObjToObjIds_->end()) {
        objectId = mapIter->second;
      }
    }
    ...
    auto aoIter = existingArticulatedObjects_.find(objectId);
    if (aoIter != existingArticulatedObjects_.end()) {
      isKinematic =
          (aoIter->second->getMotionType() == MotionType::KINEMATIC);
    } else {
      auto roIter = existingObjects_.find(objectId);
      if (roIter != existingObjects_.end()) {
        isKinematic =
            (roIter->second->getMotionType() == MotionType::KINEMATIC);
      }
    }
    if (!isKinematic) {
      continue;
    ...
    appendCapsulesFromShape(shape, obj->getWorldTransform(), obstacles);
  }
...
void BulletPhysicsManager::debugDrawCollisionGroup(
    const Magnum::Matrix4& projTrans, uint32_t groupMask) const {
  if (!debugDrawer_ || !bWorld_) {
    return;
  }

  debugDrawer_->setTransformationProjectionMatrix(projTrans);

  const btVector3 color(0.2f, 0.8f, 0.3f);
  const int numObjects = bWorld_->getNumCollisionObjects();
  for (int i = 0; i < numObjects; ++i) {
    btCollisionObject* obj = bWorld_->getCollisionObjectArray()[i];
    if (!obj) {
    ...
    btBroadphaseProxy* handle = obj->getBroadphaseHandle();
    if (!handle) {
      continue;
    }
    if ((handle->m_collisionFilterGroup & groupMask) == 0) {
      continue;
    }
    bWorld_->debugDrawObject(obj->getWorldTransform(),
                             obj->getCollisionShape(), color);
  }

  btIDebugDraw* drawer = debugDrawer_.get();
  drawer->flushLines();
}
