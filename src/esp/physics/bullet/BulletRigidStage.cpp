...
void BulletRigidStage::constructBulletSceneFromMeshes(
  ...
  if (mesh) {
    ...
    const int numTriangles = static_cast<int>(ui_data.size() / 3);
    bulletMesh.m_numTriangles = numTriangles;
    ...
    constexpr int maxQuantizedTriangles =
        (1 << (31 - MAX_NUM_PARTS_IN_BITS)) - 1;
    const bool useQuantized = numTriangles <= maxQuantizedTriangles;
    if (!useQuantized) {
      ESP_WARNING()
          << "Collision mesh has" << numTriangles
          << "triangles; disabling quantized BVH to avoid Bullet part-id "
             "overflow (max"
          << maxQuantizedTriangles << ").";
    }
