...
class BulletPhysicsManager : public PhysicsManager {
 public:
   ...
   */
  std::vector<CapsuleObstacle> getKinematicCapsuleObstacles() const override;
  ...
  /**
   * @brief Render collision wireframes for objects in the specified collision
   * group mask, skipping other collision objects (e.g., the static stage).
   * @param projTrans The composed projection and transformation matrix for the
   * render camera.
   * @param groupMask Bitmask of @ref CollisionGroup values to draw.
   */
  void debugDrawCollisionGroup(const Magnum::Matrix4& projTrans,
