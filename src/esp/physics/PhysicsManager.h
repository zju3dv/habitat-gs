...
/**
 * @brief Capsule obstacle representation for navmesh dynamic collision checks.
 */
struct CapsuleObstacle {
  Magnum::Vector3 p0;
  Magnum::Vector3 p1;
  float radius = 0.0f;
};
...
class PhysicsManager : public std::enable_shared_from_this<PhysicsManager> {
 public:
  ...
  /**
   * @brief Gaussian time cursor used to slice 4DGS proxies.
   */
  virtual void setGaussianTime(float time) { gaussianTime_ = time; }
  ...
  /**
   * @brief Collect kinematic capsule obstacles (e.g., avatar proxy URDFs).
   *
   * Default implementation returns an empty list for non-bullet backends.
   */
  virtual std::vector<CapsuleObstacle> getKinematicCapsuleObstacles() const {
    return {};
  }
