...
// Bring Corrade container iteration functions into scope
using Corrade::Containers::begin;
using Corrade::Containers::end;
...
#include "esp/gfx/GaussianSplattingShader.h"
...
struct Renderer::Impl {
  ...
  void draw(RenderCamera& camera,
    ...
    RenderCamera::DrawableTransforms gaussianDrawables;
    RenderCamera::DrawableTransforms otherDrawables;
    ...
    for (auto& it : sceneGraph.getDrawableGroups()) {
      ...
      if (it.second.prepareForDraw(camera) || true) {
        ...
        for (auto& entry : transforms) {
          if (dynamic_cast<GaussianSplattingDrawable*>(
                  &entry.first.get()) != nullptr) {
            gaussianDrawables.emplace_back(entry);
          } else {
    ...
    if (!otherDrawables.empty()) {
      camera.draw(otherDrawables, flags);
    }
    if (!gaussianDrawables.empty()) {
      camera.draw(gaussianDrawables, flags);
    }
  }
  ...
  void draw(sensor::VisualSensor& visualSensor, sim::Simulator& sim) {
    ...
    auto* renderCamera = visualSensor.getRenderCamera();
    if (renderCamera && visualSensor.hasRenderTarget()) {
      renderCamera->setRenderTarget(&visualSensor.renderTarget());
      renderCamera->setDepthRange(visualSensor.specification()->near,
                                  visualSensor.specification()->far);
      renderCamera->setGaussianTime(sim.getGaussianTime());
      auto shader = getShader<GaussianSplattingShader>(
          RendererShaderType::GaussianSplattingDepth);
      renderCamera->setGaussianSplattingShader(shader.operator->());
    }
#endif
...
#ifdef ESP_BUILD_WITH_CUDA
        // Ensure color targets have a texture depth attachment so depth can be
        // safely fanned out to same-pose depth sensors via FBO blit.
  ...
  Mn::Resource<Mn::GL::AbstractShaderProgram, T> getShader(
    ...
    switch (type) {
      ...
      case RendererShaderType::GaussianFilter:
        ...
        break;
      case RendererShaderType::GaussianSplattingDepth:
        key = Mn::ResourceKey{"gaussianSplattingDepth"};
        break;
    ...
    if (!shader) {
      if (type == RendererShaderType::DepthShader) {
        ...
        shaderManager_.set<Mn::GL::AbstractShaderProgram>(
            ...
            Mn::ResourceDataState::Final, Mn::ResourcePolicy::Resident);
      } else if (type == RendererShaderType::GaussianSplattingDepth) {
        shaderManager_.set<Mn::GL::AbstractShaderProgram>(
            shader.key(), new GaussianSplattingShader{},
            Mn::ResourceDataState::Final, Mn::ResourcePolicy::Resident);
      }
    }
