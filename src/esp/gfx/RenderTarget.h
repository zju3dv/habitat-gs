...
class RenderTarget {
 public:
  ...
  /**
   * @brief Blits the rgba buffer from this render target to another render
   * target.
   * @return true if blit succeeded, false otherwise (e.g. size/attachment
   * mismatch).
   */
  bool blitRgbaTo(RenderTarget& target);

  /**
   * @brief Blits the depth buffer from this render target to another render
   * target.
   * @return true if blit succeeded, false otherwise (e.g. size mismatch).
   */
  bool blitDepthTo(RenderTarget& target);
  ...
  /**
   * @brief get the color texture (for CUDA-GL interop)
   */
  Magnum::GL::Texture2D& getColorTexture();

#ifdef ESP_BUILD_WITH_CUDA
  /**
   * @brief Blend Gaussian Splatting outputs into the primary framebuffer,
   * respecting the existing depth buffer.
   */
  void compositeGaussian(GaussianSplattingShader& shader, bool hasColor);
#endif

  /**
   * @brief Whether the last render pass into this target performed Gaussian
   * composite.
   */
...
#ifdef ESP_BUILD_WITH_CUDA
  /**
   * @brief get the CUDA-compatible depth texture (R32F format)
  ...
  /**
   * @brief get the CUDA-compatible color texture dedicated to Gaussian
   * splatting (RGBA32F format).
   */
  ...
  /**
   * @brief Check if this render target has a color attachment
   * @return true if the render target was created with RgbaAttachment flag
   */
