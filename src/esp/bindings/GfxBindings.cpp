...
void initRenderTargetBind(py::module& m) {
  ...
  pyRenderTarget
      ...
      .def("blit_rgba_to",
           [](RenderTarget& self, RenderTarget& target) {
             return self.blitRgbaTo(target);
           })
      .def("blit_depth_to",
           [](RenderTarget& self, RenderTarget& target) {
             return self.blitDepthTo(target);
           })
      .def_property_readonly("had_gaussian_composite_in_last_pass",
                             &RenderTarget::hadGaussianCompositeInLastPass)
