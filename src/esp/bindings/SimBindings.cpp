...
void initSimConfigBindings(py::module& m) {
  ...
  py::class_<SimulatorConfiguration, SimulatorConfiguration::ptr>(
      ...
      .def_readwrite(
          "gaussian_time", &SimulatorConfiguration::gaussianTime,
          R"(Global time cursor forwarded to Gaussian splatting renderers (seconds).)")
      .def_readwrite(
          "gaussian_auto_play", &SimulatorConfiguration::autoPlayGaussian,
          R"(Advance Gaussian time automatically during step_world for dynamic splats.)")
      .def_readwrite(
          "gaussian_time_min", &SimulatorConfiguration::gaussianTimeMin,
          R"(Lower bound for Gaussian playback (seconds).)")
      .def_readwrite(
          "gaussian_time_max", &SimulatorConfiguration::gaussianTimeMax,
          R"(Upper bound for Gaussian playback; <=0 disables clamping.)")
      .def_readwrite(
          "gaussian_time_step", &SimulatorConfiguration::gaussianTimeStep,
          R"(Fixed Gaussian playback step in seconds; <=0 uses the physics or render dt.)")
      .def_readwrite(
          "gaussian_time_loop", &SimulatorConfiguration::loopGaussianTime,
          R"(Wrap Gaussian time when exceeding the upper bound instead of clamping.)")
      .def_readwrite(
...
void initSimBindings(py::module& m) {
  ...
  py::class_<Simulator, Simulator::ptr>(m, "Simulator")
      ...
      .def(
          "physics_debug_draw_collision_group",
          &Simulator::physicsDebugDrawCollisionGroup, "projMat"_a,
          "group_mask"_a,
          R"(Render Bullet collision wireframes for objects matching a collision group mask. Useful for drawing kinematic proxies without static stage meshes.)")
      ...
      .def_property("gaussian_time", &Simulator::getGaussianTime,
                    &Simulator::setGaussianTime,
                    R"(Global time cursor forwarded to Gaussian splatting renderers.)")
      .def_property(
          "gaussian_time_min", &Simulator::getGaussianTimeMin,
          &Simulator::setGaussianTimeMin,
          R"(Lower bound for Gaussian playback; ignored when >= gaussian_time_max.)")
      .def_property(
          "gaussian_time_max", &Simulator::getGaussianTimeMax,
          &Simulator::setGaussianTimeMax,
          R"(Upper bound for Gaussian playback; values <= min disable clamping.)")
      .def_property(
          "gaussian_time_step", &Simulator::getGaussianTimeStep,
          &Simulator::setGaussianTimeStep,
          R"(Override Gaussian playback step (seconds). <=0 uses dt.)")
      .def_property("gaussian_time_loop", &Simulator::getLoopGaussianTime,
                    &Simulator::setLoopGaussianTime,
                    R"(Whether Gaussian playback wraps around when exceeding max.)")
      .def_property("gaussian_auto_play", &Simulator::getAutoPlayGaussian,
                    &Simulator::setAutoPlayGaussian,
                    R"(Automatically advance Gaussian time each step for dynamic Gaussian assets and avatars.)")
      .def_property_readonly("gaussian_duration", &Simulator::getGaussianDuration,
                             R"(Duration metadata parsed from the active Gaussian asset, if available.)")
      .def_property_readonly("gaussian_interval", &Simulator::getGaussianInterval,
                             R"(Frame interval metadata parsed from the active Gaussian asset, if available.)")
      .def_property(
      ...
      .def(
          "get_gaussian_avatar_ids", &Simulator::getGaussianAvatarIds,
          R"(Return IDs for Gaussian Avatar instances in the current scene.)")
      .def(
          "update_gaussian_avatar",
          [](Simulator& sim, int avatar_id, size_t joint_mats_ptr,
             size_t offsets_ptr, size_t colors_ptr, size_t scales_ptr) {
            return sim.updateGaussianAvatar(avatar_id, joint_mats_ptr,
                                            offsets_ptr, colors_ptr,
                                            scales_ptr);
          },
          R"(Update Gaussian Avatar pose and attributes from CUDA device pointers.)",
          "avatar_id"_a, "joint_mats_ptr"_a, "offsets_ptr"_a = 0,
          "colors_ptr"_a = 0, "scales_ptr"_a = 0)
      .def(
          "update_gaussian_avatar_attributes",
          [](Simulator& sim, int avatar_id, size_t offsets_ptr,
             size_t colors_ptr, size_t scales_ptr) {
            return sim.updateGaussianAvatarAttributes(
                avatar_id, offsets_ptr, colors_ptr, scales_ptr);
          },
          R"(Update Gaussian Avatar attributes from CUDA device pointers.)",
          "avatar_id"_a, "offsets_ptr"_a, "colors_ptr"_a, "scales_ptr"_a)
      .def(
          "update_gaussian_avatar_pose",
          [](Simulator& sim, int avatar_id, size_t joint_mats_ptr) {
            return sim.updateGaussianAvatarPose(avatar_id, joint_mats_ptr);
          },
          R"(Update Gaussian Avatar pose from a CUDA device pointer.)",
          "avatar_id"_a, "joint_mats_ptr"_a)
      .def(
          "update_gaussian_avatar_bounds",
          [](Simulator& sim, int avatar_id, const py::array_t<float>& bounds_min,
             const py::array_t<float>& bounds_max) {
            if (!is_contiguous(bounds_min) || !is_contiguous(bounds_max)) {
              throw std::runtime_error(
                  "update_gaussian_avatar_bounds expects contiguous float32 "
                  "arrays.");
            }

            const py::buffer_info minInfo = bounds_min.request();
            const py::buffer_info maxInfo = bounds_max.request();

            auto validate = [](const py::buffer_info& info) {
              return (info.ndim == 1 && info.shape[0] == 3) ||
                     (info.ndim == 2 && info.shape[0] == 1 &&
                      info.shape[1] == 3);
            };
            if (!validate(minInfo) || !validate(maxInfo)) {
              throw std::runtime_error(
                  "update_gaussian_avatar_bounds expects shape [3] or [1, 3] "
                  "for both bounds.");
            }

            const auto* minData = static_cast<const float*>(minInfo.ptr);
            const auto* maxData = static_cast<const float*>(maxInfo.ptr);
            return sim.updateGaussianAvatarBounds(
                avatar_id,
                Magnum::Vector3{minData[0], minData[1], minData[2]},
                Magnum::Vector3{maxData[0], maxData[1], maxData[2]});
          },
          R"(Update world-space frustum culling bounds for a Gaussian Avatar from float32 arrays.)",
          "avatar_id"_a, "bounds_min"_a, "bounds_max"_a)
      .def(
          "set_gaussian_avatar_visible", &Simulator::setGaussianAvatarVisible,
          R"(Toggle Gaussian Avatar visibility in the active drawable group.)",
          "avatar_id"_a, "visible"_a)
      .def(
          "set_navmesh_dynamic_capsules",
          [](Simulator& sim, const py::array_t<float>& capsulesArray) {
            if (!is_contiguous(capsulesArray)) {
              throw std::runtime_error(
                  "set_navmesh_dynamic_capsules expects a contiguous float32 "
                  "array.");
            }
            py::buffer_info info = capsulesArray.request();
            if (info.ndim != 2 || info.shape[1] != 7) {
              throw std::runtime_error(
                  "set_navmesh_dynamic_capsules expects shape [N, 7].");
            }

            const auto count = static_cast<size_t>(info.shape[0]);
            const auto* data = static_cast<const float*>(info.ptr);
            std::vector<esp::nav::DynamicCapsule> capsules;
            capsules.reserve(count);
            for (size_t i = 0; i < count; ++i) {
              const size_t base = i * 7;
              esp::nav::DynamicCapsule cap;
              cap.p0 = Magnum::Vector3{data[base], data[base + 1],
                                       data[base + 2]};
              cap.p1 = Magnum::Vector3{data[base + 3], data[base + 4],
                                       data[base + 5]};
              cap.radius = data[base + 6];
              capsules.push_back(cap);
            }
            sim.setNavmeshDynamicCapsules(capsules);
          },
          R"(Set externally driven dynamic navmesh capsules as a float32 array with shape [N, 7] and rows [p0.x, p0.y, p0.z, p1.x, p1.y, p1.z, radius].)",
          "capsules"_a)
      .def("clear_navmesh_dynamic_capsules",
           &Simulator::clearNavmeshDynamicCapsules,
           R"(Clear externally driven dynamic navmesh capsules.)")
      .def(
          "distance_to_avatar_proxy_capsules",
          &Simulator::distanceToAvatarProxyCapsules,
          R"(Return signed clearance between the agent capsule and externally driven avatar proxy capsules.
          Positive means separated, zero means touching, negative means overlapping.
          Returns +inf when no avatar proxy capsules are active.)",
          "agent_base_pos"_a, "agent_radius"_a = -1.0f,
          "agent_height"_a = -1.0f)
      .def(
          "is_step_blocked_by_avatar_proxy_capsules",
          &Simulator::isStepBlockedByAvatarProxyCapsules,
          R"(Return true if avatar proxy capsules block motion from start to end on the navmesh.
          This compares navmesh-only motion against avatar-capsule-clipped motion and reports a block only when clipping reduces displacement beyond block_eps.)",
          "start"_a, "end"_a, "allow_sliding"_a = true,
          "agent_radius"_a = -1.0f, "agent_height"_a = -1.0f,
          "block_eps"_a = 1.0e-5f)
