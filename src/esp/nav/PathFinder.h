...
/**
 * @brief Capsule representation for dynamic obstacles.
 */
...
class PathFinder {
 public:
  ...
  /**
   * @brief Compute signed clearance between an agent capsule and a list of
   * capsule obstacles.
   *
   * Clearance is positive when separated, zero when touching, and negative
   * when overlapping. Returns +inf when @p capsules is empty or dimensions are
   * invalid.
   */
  float distanceToCapsules(const Magnum::Vector3& basePos,
                           const std::vector<DynamicCapsule>& capsules,
                           float agentRadius,
                           float agentHeight) const;

  /**
   * @brief Check whether capsule obstacles block a motion on the navmesh.
   *
   * This compares navmesh-only motion against navmesh motion additionally
   * clipped by @p capsules. Returns true only when capsule clipping shortens
   * the feasible displacement by more than @p blockEps.
   */
  bool isStepBlockedByCapsules(const Magnum::Vector3& start,
  ...
  /**
   * @brief Set dynamic capsule obstacles for collision-aware tryStep.
   */
  void setDynamicCapsules(const std::vector<DynamicCapsule>& capsules);

  /**
   * @brief Clear any dynamic capsule obstacles.
   */
  void clearDynamicCapsules();
