...
float segmentSegmentDistanceSquared(const Mn::Vector3& p1,
                                    const Mn::Vector3& q1,
                                    const Mn::Vector3& p2,
                                    const Mn::Vector3& q2) {
  const Mn::Vector3 d1 = q1 - p1;
  const Mn::Vector3 d2 = q2 - p2;
  const Mn::Vector3 r = p1 - p2;
  const float a = Mn::Math::dot(d1, d1);
  const float e = Mn::Math::dot(d2, d2);
  const float f = Mn::Math::dot(d2, r);
  ...
  if (a <= kEps && e <= kEps) {
    return Mn::Math::dot(r, r);
  }
  if (a <= kEps) {
    t = Mn::Math::clamp(f / e, 0.0f, 1.0f);
  } else {
    const float c = Mn::Math::dot(d1, r);
    if (e <= kEps) {
      s = Mn::Math::clamp(-c / a, 0.0f, 1.0f);
    } else {
      const float b = Mn::Math::dot(d1, d2);
      const float denom = a * e - b * b;
      if (denom != 0.0f) {
        s = Mn::Math::clamp((b * f - c * e) / denom, 0.0f, 1.0f);
      }
      t = (b * s + f) / e;
      if (t < 0.0f) {
        t = 0.0f;
        ...
        t = 1.0f;
        s = Mn::Math::clamp((b - c) / a, 0.0f, 1.0f);
      }
    }
  }

  const Mn::Vector3 c1 = p1 + d1 * s;
  const Mn::Vector3 c2 = p2 + d2 * t;
  return Mn::Math::dot(c1 - c2, c1 - c2);
}

bool collidesAtPosition(const std::vector<DynamicCapsule>& capsules,
                        const Mn::Vector3& basePos,
                        float agentRadius,
                        float agentHeight) {
  const Mn::Vector3 a0 = basePos;
  const Mn::Vector3 a1 = basePos + Mn::Vector3{0.0f, agentHeight, 0.0f};
  const float radius = agentRadius;
  for (const auto& capsule : capsules) {
    const float sumRadius = radius + capsule.radius;
    const float dist2 = segmentSegmentDistanceSquared(a0, a1, capsule.p0,
                                                      capsule.p1);
...
float capsuleClearanceAtPosition(const std::vector<DynamicCapsule>& capsules,
                                 const Mn::Vector3& basePos,
                                 float agentRadius,
                                 float agentHeight) {
  if (capsules.empty() || agentRadius <= 0.0f || agentHeight <= 0.0f) {
  ...
  for (const auto& capsule : capsules) {
    const float sumRadius = agentRadius + capsule.radius;
    const float dist2 =
        segmentSegmentDistanceSquared(a0, a1, capsule.p0, capsule.p1);
    const float centerDist = std::sqrt(Mn::Math::max(0.0f, dist2));
    minClearance = Mn::Math::min(minClearance, centerDist - sumRadius);
  }
  return minClearance;
}

Mn::Vector3 clipDynamicCapsules(const std::vector<DynamicCapsule>& capsules,
                                const Mn::Vector3& start,
  ...
  const Mn::Vector3 delta = end - start;
  const float len = delta.length();
  if (len <= 1.0e-6f) {
    return end;
  }

  const float stepSize = Mn::Math::max(0.05f, agentRadius * 0.5f);
  const int steps =
      std::min(10, std::max(1, int(std::ceil(len / stepSize))));

  float lastSafe = 0.0f;
  for (int i = 1; i <= steps; ++i) {
    const float t = static_cast<float>(i) / static_cast<float>(steps);
    const Mn::Vector3 pos = start + delta * t;
    if (collidesAtPosition(capsules, pos, agentRadius, agentHeight)) {
      float lo = lastSafe;
      float hi = t;
      for (int iter = 0; iter < 6; ++iter) {
        const float mid = 0.5f * (lo + hi);
        const Mn::Vector3 midPos = start + delta * mid;
        if (collidesAtPosition(capsules, midPos, agentRadius, agentHeight)) {
          hi = mid;
        } else {
          lo = mid;
        }
      }
      const float backoff = 1.0e-4f;
      const float safeT = Mn::Math::max(0.0f, lo - backoff);
...
struct PathFinder::Impl {
  ...
  template <typename T>
  bool computeNavmeshStepEndpoint(const T& start,
                                  const T& end,
                                  bool allowSliding,
                                  Mn::Vector3& endPoint) const;
  ...
  HitRecord closestObstacleSurfacePoint(const Mn::Vector3& pt,
                                        float maxSearchRadius = 2.0f) const;
  float distanceToCapsules(const Mn::Vector3& basePos,
                           const std::vector<DynamicCapsule>& capsules,
                           float agentRadius,
                           float agentHeight) const;
  bool isStepBlockedByCapsules(const Mn::Vector3& start,
                               const Mn::Vector3& end,
                               const std::vector<DynamicCapsule>& capsules,
                               float agentRadius,
                               float agentHeight,
  ...
  void setDynamicCapsules(const std::vector<DynamicCapsule>& capsules) {
    dynamicCapsules_ = capsules;
  }
 ...
 private:
  ...
  std::vector<DynamicCapsule> dynamicCapsules_;
...
template <typename T>
bool PathFinder::Impl::computeNavmeshStepEndpoint(const T& start,
                                                  const T& end,
                                                  bool allowSliding,
                                                  Mn::Vector3& endPoint) const {
  static const int MAX_POLYS = 256;
  ...
  navQuery_->moveAlongSurface(startRef, pathStart.data(), end.data(),
                              filter_.get(), endPoint.data(), polys,
                              &numPolys, MAX_POLYS);
...
T PathFinder::Impl::tryStep(const T& start, const T& end, bool allowSliding) {
  Mn::Vector3 endPoint;
  if (!computeNavmeshStepEndpoint(start, end, allowSliding, endPoint)) {
    return start;
  }

  if (!dynamicCapsules_.empty() && navMeshSettings_) {
    const Mn::Vector3 startVec{start};
    endPoint = clipDynamicCapsules(dynamicCapsules_, startVec, endPoint,
                                   navMeshSettings_->agentRadius,
                                   navMeshSettings_->agentHeight);
  }
...
float PathFinder::Impl::distanceToCapsules(
    const Mn::Vector3& basePos,
    const std::vector<DynamicCapsule>& capsules,
    const float agentRadius,
    const float agentHeight) const {
  return capsuleClearanceAtPosition(capsules, basePos, agentRadius, agentHeight);
}

bool PathFinder::Impl::isStepBlockedByCapsules(
    const Mn::Vector3& start,
    const Mn::Vector3& end,
    const std::vector<DynamicCapsule>& capsules,
    const float agentRadius,
    ...
    const float blockEps) const {
  if (capsules.empty() || agentRadius <= 0.0f || agentHeight <= 0.0f) {
    return false;
  }

  Mn::Vector3 navmeshEnd;
  if (!computeNavmeshStepEndpoint(start, end, allowSliding, navmeshEnd)) {
    return false;
  }

  const float navmeshMove2 = (navmeshEnd - start).dot();
  if (navmeshMove2 <= blockEps) {
    return false;
  }

  const Mn::Vector3 clippedEnd =
      clipDynamicCapsules(capsules, start, navmeshEnd, agentRadius, agentHeight);
  const float clippedMove2 = (clippedEnd - start).dot();
  return (clippedMove2 + blockEps) < navmeshMove2;
}
...
float PathFinder::distanceToCapsules(
    const Mn::Vector3& basePos,
    const std::vector<DynamicCapsule>& capsules,
    const float agentRadius,
    const float agentHeight) const {
  return pimpl_->distanceToCapsules(basePos, capsules, agentRadius, agentHeight);
}

bool PathFinder::isStepBlockedByCapsules(
    const Mn::Vector3& start,
    const Mn::Vector3& end,
    const std::vector<DynamicCapsule>& capsules,
    const float agentRadius,
    ...
    const bool allowSliding,
    const float blockEps) const {
  return pimpl_->isStepBlockedByCapsules(start, end, capsules, agentRadius,
                                         agentHeight, allowSliding, blockEps);
}
...
void PathFinder::setDynamicCapsules(
    const std::vector<DynamicCapsule>& capsules) {
  pimpl_->setDynamicCapsules(capsules);
}

void PathFinder::clearDynamicCapsules() {
  pimpl_->setDynamicCapsules({});
}
