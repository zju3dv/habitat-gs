...
void CameraSensor::recomputeProjectionMatrix() {
  ...
  renderCamera_->setProjectionMatrix(cameraSensorSpec_->resolution[1],
                                     ...
                                     projectionMatrix_);
  renderCamera_->setDepthRange(cameraSensorSpec_->near, cameraSensorSpec_->far);
}
...
bool CameraSensor::drawObservation(sim::Simulator& sim) {
  ...
  renderCamera_->setGaussianTime(sim.getGaussianTime());
  ...
  if (cameraSensorSpec_->sensorType == SensorType::Semantic) {
    ...
    gfx::DrawableGroup& defaultRenderingGroup = sceneGraph.getDrawables();
    defaultRenderingGroup.prepareForDraw(*renderCamera_);
    auto drawableTransforms =
        renderCamera_->drawableTransformations(defaultRenderingGroup);
    renderCamera_->filterTransforms(drawableTransforms, flags);

    gfx::RenderCamera::DrawableTransforms gaussianDrawables;
    gfx::RenderCamera::DrawableTransforms otherDrawables;
    gaussianDrawables.reserve(drawableTransforms.size());
    otherDrawables.reserve(drawableTransforms.size());
    for (auto& entry : drawableTransforms) {
      if (dynamic_cast<gfx::GaussianSplattingDrawable*>(
    ...
    const bool renderedOpaque = !otherDrawables.empty();
    if (renderedOpaque) {
      renderCamera_->draw(otherDrawables, flags);
    }

    if (cameraSensorSpec_->sensorType == SensorType::Color && renderedOpaque) {
      // include HBAO in Color sensors (only if enabled for render target)
      renderTarget().tryDrawHbao();
    }

    if (!gaussianDrawables.empty()) {
      renderCamera_->draw(gaussianDrawables, flags);
    }
