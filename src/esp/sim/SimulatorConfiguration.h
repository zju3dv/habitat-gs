...
struct SimulatorConfiguration {
  ...
  //! Global time cursor for dynamic Gaussian splatting assets (seconds).
  float gaussianTime = 0.0f;
  //! Auto-play Gaussian time in stepWorld for dynamic splats.
  bool autoPlayGaussian = true;
  //! Whether Gaussian time should wrap when exceeding the max bound.
  bool loopGaussianTime = true;
  //! Optional lower bound for Gaussian playback.
  float gaussianTimeMin = 0.0f;
  //! Optional upper bound for Gaussian playback (0 disables clamping).
  float gaussianTimeMax = 0.0f;
  //! Optional fixed time step for Gaussian playback (seconds).
  float gaussianTimeStep = 0.0f;
