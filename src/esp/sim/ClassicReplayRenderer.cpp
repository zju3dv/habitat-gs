...
void ClassicReplayRenderer::doRender(
  ...
  for (int envIndex = 0; envIndex < config_.numEnvironments; envIndex++) {
    ...
    bool renderedOpaque = false;
    std::vector<gfx::RenderCamera::DrawableTransforms> gaussianPass;

    for (auto& group : sceneGraph.getDrawableGroups()) {
      group.second.prepareForDraw(*camera);
      auto transforms = camera->drawableTransformations(group.second);
      ...
      gaussianDrawables.reserve(transforms.size());
      otherDrawables.reserve(transforms.size());

      for (auto& entry : transforms) {
        if (dynamic_cast<gfx::GaussianSplattingDrawable*>(
                &entry.first.get()) != nullptr ||
            dynamic_cast<gfx::GaussianAvatarDrawable*>(
                &entry.first.get()) != nullptr) {
      ...
      if (!gaussianDrawables.empty()) {
        gaussianPass.emplace_back(std::move(gaussianDrawables));
      }
    }
    ...
    for (auto& transforms : gaussianPass) {
