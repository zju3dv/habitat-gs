...
class Simulator {
 public:
  ...
  /**
   * @brief Time cursor used for dynamic Gaussian splatting (seconds).
   */
  float getGaussianTime() const { return gaussianTime_; }
  void setGaussianTime(float time);
  float getGaussianTimeMin() const { return gaussianTimeMin_; }
  void setGaussianTimeMin(float time);
  float getGaussianTimeMax() const { return gaussianTimeMax_; }
  void setGaussianTimeMax(float time);
  float getGaussianTimeStep() const { return gaussianTimeStep_; }
  void setGaussianTimeStep(float timeStep);
  bool getLoopGaussianTime() const { return loopGaussianTime_; }
  void setLoopGaussianTime(bool loop);
  bool getAutoPlayGaussian() const { return autoPlayGaussian_; }
  void setAutoPlayGaussian(bool autoPlay);
  float getGaussianDuration() const { return gaussianDuration_; }
  float getGaussianInterval() const { return gaussianInterval_; }
  bool isGaussian4D() const { return gaussianIs4D_; }

  /**
   * @brief Return IDs of all Gaussian Avatar instances in the current scene.
   */
  std::vector<int> getGaussianAvatarIds() const;

  /**
   * @brief Update Gaussian Avatar attributes and pose using device pointers.
   */
  bool updateGaussianAvatar(int avatarId,
  ...
  /**
   * @brief Update Gaussian Avatar attributes using device pointers.
   */
  bool updateGaussianAvatarAttributes(int avatarId,
                                      size_t offsetsPtr,
                                      size_t colorsPtr,
                                      size_t scalesPtr);

  /**
   * @brief Update Gaussian Avatar pose using device pointer.
   */
  bool updateGaussianAvatarPose(int avatarId, size_t jointMatricesPtr);

  /**
   * @brief Update world-space frustum culling bounds for a Gaussian Avatar.
   */
  bool updateGaussianAvatarBounds(int avatarId,
                                  const Magnum::Vector3& boundsMin,
                                  const Magnum::Vector3& boundsMax);

  /**
   * @brief Toggle Gaussian Avatar visibility in the active drawable group.
   */
  bool setGaussianAvatarVisible(int avatarId, bool visible);

  /**
   * @brief Set externally driven dynamic capsule obstacles for navmesh
   * collision checks (e.g., Gaussian avatar proxy capsules).
   */
  void setNavmeshDynamicCapsules(
      const std::vector<nav::DynamicCapsule>& capsules);

  /**
   * @brief Clear externally driven dynamic capsule obstacles.
   */
  void clearNavmeshDynamicCapsules();

  /**
   * @brief Compute signed clearance between an agent capsule and externally
   * driven avatar proxy capsules.
   *
   * Returns positive when separated, zero when touching, negative when
   * overlapping, and +inf when no avatar proxy capsules are active.
   */
  float distanceToAvatarProxyCapsules(const Magnum::Vector3& agentBasePos,
                                      float agentRadius = -1.0f,
                                      float agentHeight = -1.0f) const;

  /**
   * @brief Query whether externally driven avatar proxy capsules block a
   * navmesh motion from @p start to @p end.
   *
   * This ignores static-navmesh clipping when classifying avatar collision:
   * it only returns true if avatar capsule clipping reduces motion beyond
   * @p blockEps.
   */
  bool isStepBlockedByAvatarProxyCapsules(
      const Magnum::Vector3& start,
      const Magnum::Vector3& end,
      bool allowSliding = true,
      float agentRadius = -1.0f,
      float agentHeight = -1.0f,
      float blockEps = 1.0e-5f) const;
  ...
  /**
   * @brief Render debugging visualizations for collision objects that match
   * the provided collision group mask. This can be used to skip static meshes.
   * @param projTrans The composed projection and transformation matrix for the
   * render camera.
   * @param groupMask Bitmask of @ref physics::CollisionGroup values to draw.
   */
  void physicsDebugDrawCollisionGroup(const Magnum::Matrix4& projTrans,
 ...
 protected:
  ...
  /**
   * @brief Sync dynamic capsule obstacles (external + physics) into the
   * navmesh pathfinder.
   */
  void syncNavmeshDynamicCapsules();
  ...
  /**
   * @brief Instance all Gaussian Avatar renderables for the current scene.
   */
  bool instanceGaussianAvatarsForSceneAttributes(
      const metadata::attributes::SceneInstanceAttributes::cptr&
          curSceneInstanceAttributes);
  ...
  void applyGaussianStageConfig(
      const metadata::attributes::StageAttributes& stageAttributes);
  float applyGaussianTimeBounds(float time) const;
  float computeGaussianDeltaTime(double dt) const;
  void autoStepGaussianTime(double dt);
  ...
  struct GaussianAvatarInstance {
    int id = ID_UNDEFINED;
    std::string name;
    std::shared_ptr<assets::GaussianAvatarData> data;
    gfx::GaussianAvatarDrawable* drawable = nullptr;
    scene::SceneNode* node = nullptr;
    bool visible = true;
  };

  std::vector<GaussianAvatarInstance> gaussianAvatarInstances_;
  int nextGaussianAvatarId_ = 0;
  std::vector<nav::DynamicCapsule> externalNavmeshCapsules_;
  ...
  float gaussianTime_ = 0.0f;
  float gaussianTimeMin_ = 0.0f;
  float gaussianTimeMax_ = 0.0f;
  float gaussianTimeStep_ = 0.0f;
  float gaussianDuration_ = 0.0f;
  float gaussianInterval_ = 0.0f;
  bool autoPlayGaussian_ = false;
  bool loopGaussianTime_ = true;
  bool gaussianIs4D_ = false;
