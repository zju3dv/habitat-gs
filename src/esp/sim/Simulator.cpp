...
#include "esp/assets/GaussianAvatarImporter.h"
...
bool resolveAgentCapsuleDimensions(const nav::PathFinder::ptr& pathfinder,
                                   float requestedRadius,
                                   float requestedHeight,
                                   float& outRadius,
                                   float& outHeight) {
  outRadius = requestedRadius;
  outHeight = requestedHeight;
  if ((!pathfinder) || (outRadius > 0.0f && outHeight > 0.0f)) {
    return outRadius > 0.0f && outHeight > 0.0f;
  }

  const auto settings = pathfinder->getNavMeshSettings();
  if (!settings) {
    return false;
  }
  if (outRadius <= 0.0f) {
    outRadius = settings->agentRadius;
  }
  if (outHeight <= 0.0f) {
    outHeight = settings->agentHeight;
  }
  return outRadius > 0.0f && outHeight > 0.0f;
}

void releaseGaussianInteropResources(scene::SceneManager* sceneManager,
                                     int activeSceneId) {
  if (!sceneManager || activeSceneId == ID_UNDEFINED) {
    return;
  }

  auto& sceneGraph = sceneManager->getSceneGraph(activeSceneId);
  for (auto& groupEntry : sceneGraph.getDrawableGroups()) {
    auto& group = groupEntry.second;
    for (const auto& drawableEntry : group.getDrawableMap()) {
      auto* drawable = drawableEntry.second;
      if (!drawable) {
        continue;
      }

      if (auto* gaussianDrawable =
              dynamic_cast<gfx::GaussianSplattingDrawable*>(drawable)) {
        if (auto* renderer = gaussianDrawable->getRenderer()) {
          renderer->releaseInteropResources();
        }
        continue;
      }

      if (auto* avatarDrawable =
              dynamic_cast<gfx::GaussianAvatarDrawable*>(drawable)) {
        if (auto* renderer = avatarDrawable->getRenderer()) {
          renderer->releaseInteropResources();
        }
      }
    }
...
void Simulator::close(const bool destroy) {
  getRenderGLContext();
  releaseGaussianInteropResources(sceneManager_.get(), activeSceneID_);
  ...
  gaussianAvatarInstances_.clear();
  nextGaussianAvatarId_ = 0;
  externalNavmeshCapsules_.clear();
  ...
  gaussianTime_ = 0.0f;
  gaussianTimeMin_ = 0.0f;
  gaussianTimeMax_ = 0.0f;
  gaussianTimeStep_ = 0.0f;
  gaussianDuration_ = 0.0f;
  gaussianInterval_ = 0.0f;
  autoPlayGaussian_ = true;
  loopGaussianTime_ = true;
  gaussianIs4D_ = false;
...
// installed.
  ...
  config_ = cfg;
  gaussianTime_ = config_.gaussianTime;
  gaussianTimeMin_ = config_.gaussianTimeMin;
  gaussianTimeMax_ = config_.gaussianTimeMax;
  gaussianTimeStep_ = config_.gaussianTimeStep;
  autoPlayGaussian_ = config_.autoPlayGaussian;
  loopGaussianTime_ = config_.loopGaussianTime;
  gaussianDuration_ = 0.0f;
  gaussianInterval_ = 0.0f;
  gaussianIs4D_ = false;
...
float Simulator::applyGaussianTimeBounds(float time) const {
  if (gaussianTimeMax_ > gaussianTimeMin_) {
    if (loopGaussianTime_) {
      const float span = gaussianTimeMax_ - gaussianTimeMin_;
      float offset = std::fmod(time - gaussianTimeMin_, span);
      if (offset < 0.0f) {
        offset += span;
      }
      return gaussianTimeMin_ + offset;
    }
    return Mn::Math::clamp(time, gaussianTimeMin_, gaussianTimeMax_);
  }
  return time;
}

void Simulator::setGaussianTime(float time) {
  gaussianTime_ = applyGaussianTimeBounds(time);
  config_.gaussianTime = gaussianTime_;
  if (physicsManager_) {
    physicsManager_->setGaussianTime(gaussianTime_);
  }
}

void Simulator::setGaussianTimeMin(float time) {
  gaussianTimeMin_ = time;
  config_.gaussianTimeMin = time;
  setGaussianTime(gaussianTime_);
}

void Simulator::setGaussianTimeMax(float time) {
  gaussianTimeMax_ = time;
  config_.gaussianTimeMax = time;
  setGaussianTime(gaussianTime_);
}

void Simulator::setGaussianTimeStep(float timeStep) {
  gaussianTimeStep_ = timeStep;
  config_.gaussianTimeStep = timeStep;
}

void Simulator::setLoopGaussianTime(bool loop) {
  loopGaussianTime_ = loop;
  config_.loopGaussianTime = loop;
  setGaussianTime(gaussianTime_);
}

void Simulator::setAutoPlayGaussian(bool autoPlay) {
  autoPlayGaussian_ = autoPlay;
  config_.autoPlayGaussian = autoPlay;
}

float Simulator::computeGaussianDeltaTime(double dt) const {
  if (dt > 0.0) {
    return static_cast<float>(dt);
  }
  if (physicsManager_) {
    const float fixedStep = physicsManager_->getTimestep();
    if (fixedStep > 0.0f) {
      return fixedStep;
    }
  }
...
void Simulator::autoStepGaussianTime(double dt) {
  if (!autoPlayGaussian_) {
    return;
  }
  const float delta = computeGaussianDeltaTime(dt);
  if (delta > 0.0f) {
    setGaussianTime(gaussianTime_ + delta);
  }
}

void Simulator::applyGaussianStageConfig(
    const metadata::attributes::StageAttributes& stageAttributes) {
  gaussianIs4D_ = false;
  gaussianDuration_ = 0.0f;
  gaussianInterval_ = 0.0f;

  const std::string renderAssetPath = stageAttributes.getRenderAssetFullPath();
  if (!renderAssetPath.empty()) {
    auto meta = resourceManager_->getGaussianMetadata(renderAssetPath);
    if (meta) {
      gaussianIs4D_ = meta->is4D;
      gaussianDuration_ = meta->duration;
      gaussianInterval_ = meta->foregroundInterval > 0.0f
                              ? meta->foregroundInterval
                              : meta->interval;
      if (gaussianInterval_ <= 0.0f) {
        gaussianInterval_ = meta->interval;
      }
      if (gaussianTimeMax_ <= 0.0f && gaussianDuration_ > 0.0f) {
        gaussianTimeMax_ = gaussianTimeMin_ + gaussianDuration_;
        config_.gaussianTimeMax = gaussianTimeMax_;
      }
    }
  }
...
bool Simulator::createSceneInstance(const std::string& activeSceneName) {
  ...
  if (navmeshFileHandle.empty()) {
    ...
    if (navmeshFileLoc.empty()) {
      ...
      navmeshLoaded = pathfinder_->loadNavMesh(navmeshFileLoc);
      ESP_DEBUG() << (navmeshLoaded ? "Navmesh Loaded." : "Navmesh load error.");
    } else {
  ...
  physicsManager_->setGaussianTime(config_.gaussianTime);
  ...
  if (success) {
    ...
    if (success) {
      ...
      success = instanceArticulatedObjectsForSceneAttributes(
          curSceneInstanceAttributes_);
      if (success) {
        // 11. Load Gaussian Avatar instances as specified by Scene Instance
        // Attributes.
        success = instanceGaussianAvatarsForSceneAttributes(
            curSceneInstanceAttributes_);
        if (success) {
...
bool Simulator::instanceStageForSceneAttributes(
  ...
  ESP_CHECK(loadSuccess, Cr::Utility::formatString("Cannot load stage :{}",
                                                   stageAttributesHandle));
  applyGaussianStageConfig(*stageAttributes);
...
bool Simulator::instanceGaussianAvatarsForSceneAttributes(
    const metadata::attributes::SceneInstanceAttributes::cptr&
        curSceneInstanceAttributes_) {
  gaussianAvatarInstances_.clear();
  nextGaussianAvatarId_ = 0;
  clearNavmeshDynamicCapsules();

  if (!curSceneInstanceAttributes_) {
    return true;
  }

  const auto avatarInstances =
      curSceneInstanceAttributes_->getGaussianAvatarInstances();
  if (avatarInstances.empty()) {
    return true;
  }
  ...
  const std::string baseDir = curSceneInstanceAttributes_->getFileDirectory();
  ...
  auto resolvePath = [&](const std::string& path) -> std::string {
    if (path.empty() || isAbsolutePath(path) || baseDir.empty()) {
      return path;
    }
    return Cr::Utility::Path::join(baseDir, path);
  };

  assets::GaussianAvatarImporter importer;

  for (size_t i = 0; i < avatarInstances.size(); ++i) {
    const auto& avatarAttrs = avatarInstances[i];
    const std::string canonicalGaussians =
        resolvePath(avatarAttrs->getCanonicalGaussians());

    if (canonicalGaussians.empty()) {
      ESP_WARNING()
          << "Gaussian Avatar missing canonical gaussians path, skipping entry.";
      continue;
    }

    assets::GaussianAvatarImporter::LoadOptions loadOpts{canonicalGaussians};
    auto data = importer.load(loadOpts);
    if (!data) {
      ESP_WARNING() << "Failed to load Gaussian Avatar data from"
                    << canonicalGaussians;
      continue;
    }

    const double scale = avatarAttrs->getScale();
    if (scale > 0.0 && scale != 1.0) {
      data->applyScale(static_cast<float>(scale));
    }

    scene::SceneNode& node = rootNode.createChild();
    node.setType(scene::SceneNodeType::Object);

    gfx::DrawableConfiguration config(
        Mn::ResourceKey{}, Mn::ResourceKey{},
        metadata::attributes::ObjectInstanceShaderType::Unspecified,
        &drawables, nullptr, nullptr, nullptr);

    auto* drawable = new gfx::GaussianAvatarDrawable(node, data, config);

    GaussianAvatarInstance instance;
    instance.id = nextGaussianAvatarId_++;
    instance.name = avatarAttrs->getHandle();
    instance.data = std::move(data);
    instance.drawable = drawable;
    instance.node = &node;
    gaussianAvatarInstances_.push_back(std::move(instance));
  }
...
void Simulator::reset(bool calledAfterSceneCreate) {
  setGaussianTime(config_.gaussianTime);
...
void Simulator::performDiscreteCollisionDetection() {
  if (physicsManager_ != nullptr) {
...
void Simulator::syncNavmeshDynamicCapsules() {
  if (!pathfinder_) {
    return;
  }
  std::vector<nav::DynamicCapsule> capsules;
  capsules.reserve(externalNavmeshCapsules_.size());
  capsules.insert(capsules.end(), externalNavmeshCapsules_.begin(),
                  externalNavmeshCapsules_.end());

  if (physicsManager_) {
    const auto obstacles = physicsManager_->getKinematicCapsuleObstacles();
    capsules.reserve(capsules.size() + obstacles.size());
    for (const auto& obs : obstacles) {
      nav::DynamicCapsule cap;
      cap.p0 = obs.p0;
      cap.p1 = obs.p1;
      cap.radius = obs.radius;
      capsules.push_back(cap);
    }
  }

  if (capsules.empty()) {
    pathfinder_->clearDynamicCapsules();
    return;
  }

  pathfinder_->setDynamicCapsules(capsules);
}

void Simulator::setNavmeshDynamicCapsules(
    const std::vector<nav::DynamicCapsule>& capsules) {
  externalNavmeshCapsules_ = capsules;
  syncNavmeshDynamicCapsules();
}

void Simulator::clearNavmeshDynamicCapsules() {
  externalNavmeshCapsules_.clear();
  syncNavmeshDynamicCapsules();
}

float Simulator::distanceToAvatarProxyCapsules(
    const Magnum::Vector3& agentBasePos,
    float agentRadius,
    float agentHeight) const {
  if (!pathfinder_ || externalNavmeshCapsules_.empty()) {
    return std::numeric_limits<float>::infinity();
  }

  float resolvedRadius = agentRadius;
  float resolvedHeight = agentHeight;
  if (!resolveAgentCapsuleDimensions(pathfinder_, agentRadius, agentHeight,
                                     resolvedRadius, resolvedHeight)) {
    return std::numeric_limits<float>::infinity();
  }

  return pathfinder_->distanceToCapsules(agentBasePos, externalNavmeshCapsules_,
                                         resolvedRadius, resolvedHeight);
}

bool Simulator::isStepBlockedByAvatarProxyCapsules(
    const Magnum::Vector3& start,
    const Magnum::Vector3& end,
    bool allowSliding,
    float agentRadius,
    float agentHeight,
    float blockEps) const {
  if (!pathfinder_ || externalNavmeshCapsules_.empty()) {
    return false;
  }
  ...
  return pathfinder_->isStepBlockedByCapsules(
      start, end, externalNavmeshCapsules_, resolvedRadius, resolvedHeight,
      allowSliding, blockEps);
}

std::vector<int> Simulator::getGaussianAvatarIds() const {
  std::vector<int> ids;
  ids.reserve(gaussianAvatarInstances_.size());
  for (const auto& avatar : gaussianAvatarInstances_) {
    ids.push_back(avatar.id);
  }
  return ids;
}

bool Simulator::updateGaussianAvatar(int avatarId,
                                     size_t jointMatricesPtr,
                                     size_t offsetsPtr,
                                     size_t colorsPtr,
                                     size_t scalesPtr) {
  for (auto& avatar : gaussianAvatarInstances_) {
    if (avatar.id != avatarId || !avatar.drawable) {
      continue;
    }
    auto* renderer = avatar.drawable->getRenderer();
    if (!renderer) {
      return false;
    }
    renderer->updateAvatar(reinterpret_cast<const float*>(jointMatricesPtr),
                           reinterpret_cast<const float*>(offsetsPtr),
                           reinterpret_cast<const float*>(colorsPtr),
                           reinterpret_cast<const float*>(scalesPtr));
    return true;
  }
  return false;
}

bool Simulator::updateGaussianAvatarAttributes(int avatarId,
                                               size_t offsetsPtr,
                                               size_t colorsPtr,
                                               size_t scalesPtr) {
  for (auto& avatar : gaussianAvatarInstances_) {
    ...
    renderer->updateAttributes(reinterpret_cast<const float*>(offsetsPtr),
                               reinterpret_cast<const float*>(colorsPtr),
                               reinterpret_cast<const float*>(scalesPtr));
    return true;
  }
...
bool Simulator::updateGaussianAvatarPose(int avatarId,
                                         size_t jointMatricesPtr) {
  for (auto& avatar : gaussianAvatarInstances_) {
    if (avatar.id != avatarId || !avatar.drawable) {
      continue;
...
bool Simulator::updateGaussianAvatarBounds(int avatarId,
                                           const Magnum::Vector3& boundsMin,
                                           const Magnum::Vector3& boundsMax) {
  for (auto& avatar : gaussianAvatarInstances_) {
    if (avatar.id != avatarId || !avatar.node) {
      continue;
    }

    if (!std::isfinite(boundsMin.x()) || !std::isfinite(boundsMin.y()) ||
        !std::isfinite(boundsMin.z()) || !std::isfinite(boundsMax.x()) ||
        !std::isfinite(boundsMax.y()) || !std::isfinite(boundsMax.z())) {
      return false;
    }

    Magnum::Vector3 minCorner{
        std::min(boundsMin.x(), boundsMax.x()),
        std::min(boundsMin.y(), boundsMax.y()),
        std::min(boundsMin.z(), boundsMax.z()),
    };
    Magnum::Vector3 maxCorner{
        std::max(boundsMin.x(), boundsMax.x()),
        std::max(boundsMin.y(), boundsMax.y()),
        std::max(boundsMin.z(), boundsMax.z()),
    };

    constexpr float kMinExtent = 1.0e-3f;
    for (int axis = 0; axis < 3; ++axis) {
      const float extent = maxCorner[axis] - minCorner[axis];
      if (extent < kMinExtent) {
        const float center = 0.5f * (minCorner[axis] + maxCorner[axis]);
        minCorner[axis] = center - 0.5f * kMinExtent;
        maxCorner[axis] = center + 0.5f * kMinExtent;
      }
    }

    avatar.node->setAbsoluteAABB(Magnum::Range3D{minCorner, maxCorner});
    return true;
  }
  return false;
}

bool Simulator::setGaussianAvatarVisible(int avatarId, bool visible) {
  if (!sceneManager_ || activeSceneID_ < 0) {
    return false;
  }
  auto& drawables = sceneManager_->getSceneGraph(activeSceneID_).getDrawables();
  for (auto& avatar : gaussianAvatarInstances_) {
    if (avatar.id != avatarId || !avatar.drawable) {
      continue;
    }
    if (visible) {
      drawables.add(*avatar.drawable);
    } else {
      drawables.remove(*avatar.drawable);
    }
...
void Simulator::physicsDebugDrawCollisionGroup(const Magnum::Matrix4& projTrans,
  ...
  if (physicsManager_->getPhysicsSimulationLibrary() !=
      physics::PhysicsManager::PhysicsSimulationLibrary::Bullet) {
    return;
  }
  auto* bulletMgr =
      dynamic_cast<physics::BulletPhysicsManager*>(physicsManager_.get());
  if (!bulletMgr) {
    return;
  }
  bulletMgr->debugDrawCollisionGroup(projTrans, groupMask);
#else
  static_cast<void>(projTrans);
  static_cast<void>(groupMask);
#endif
}
...
assets::MeshData::ptr Simulator::getJoinedMesh(
  ...
  if (stageInitAttrs != nullptr) {
    // For Gaussian Splatting scenes, render asset is a *.gs/.3dgs/.4dgs.ply/.4dgs.npz which cannot be used for collision.
    // Use collision asset path if available, otherwise fall back to render asset.
    std::string collisionAssetPath = stageInitAttrs->getCollisionAssetFullPath();
    std::string meshPath = collisionAssetPath.empty() 
        ? stageInitAttrs->getRenderAssetFullPath() 
        : collisionAssetPath;
    joinedMesh = resourceManager_->createJoinedCollisionMesh(meshPath);
  }
